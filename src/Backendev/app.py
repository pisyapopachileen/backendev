...
class BackendSimulatorApp(toga.App):
    def startup(self):
        ...
        for key, text in status_data:
            status_item = toga.Box(style=Pack(direction=COLUMN))
        ...
        # Активные проекты
        active_projects_box = toga.Box(style=Pack(direction=COLUMN, flex=1))
        ...
        # Очередь проектов
        queue_box = toga.Box(style=Pack(direction=COLUMN, flex=1))
        ...
        # Кнопки действий - используем ScrollContainer для мобильной адаптации
        buttons_container = toga.ScrollContainer(horizontal=False)
        ...
        buttons_container.content = buttons_box
        
        # Сборка интерфейса с правильными отступами
        main_box.add(header)
        main_box.add(toga.Box(style=Pack(height=10)))  # Отступ
